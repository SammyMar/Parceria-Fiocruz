...
for (estado in estados){
  ...
  names(dataframe) <- c('codigo', 'ano', 'BAIXO_PESO')
...
write.csv(df, 'Indicadores dados/BAIXO_PESO.csv')
...
for (estado in estados){
  ...
  request <- POST(url = endpoint, body = params, encode = "form")
  dataframe <- convertRequestToDF(request)
  names(dataframe) <- c('codigo', 'ano', 'PREMATUROS')
  df <- rbind(df, dataframe)
...
write.csv(df, 'Indicadores dados/PREMATUROS.csv')
...
for (estado in estados){
  ...
  request <- POST(url = endpoint, body = params, encode = "form")
  dataframe <- convertRequestToDF(request)
  names(dataframe) <- c('codigo', 'ano', 'PRECOCE')
  df <- rbind(df, dataframe)
...
write.csv(df, 'Indicadores dados/PRECOCE.csv')
